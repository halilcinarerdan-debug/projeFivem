-- =====================================================================
-- MATRIX BLACK MARKET / server/blackmarket.lua  (KATMAN 5 ULTIMATE — YENİ)
--
-- Taktik Karaborsa Ticaret Ağı: illegal araç filosu, seri no silinmiş
-- silahlar, Yedek Namlu (bkz. server/forensics.lua /namludegistir) ve
-- sahte IMEI'li Açık Hat (Burner Phone, bkz. server/market.lua Matrix.
...
--   [B2] Ödeme bütünlüğü: Matrix.Fleet.RegisterVehicle veya ox_inventory
--        AddItem başarısız olursa tahsil edilen nakit OTOMATİK iade edilir
--        — oyuncu asla parasını verip karşılığında hiçbir şey alamadan
--        kalmaz.
...
RegisterNetEvent('matrix:server:blackmarket:buyWeapon', function(catalogId)
    ...
    local addOk = pcall(function()
        return exports['ox_inventory']:AddItem(src, entry.item, 1, metadata)
    end)
    if not addOk then
        RefundCash(src, entry.price)
        Reply(src, 'Silah teslim edilemedi, odeme iade edildi (envanter dolu olabilir).')
        TriggerClientEvent('matrix:client:blackmarket:purchaseResult', src, false, entry.label, nil)
        return
    end
    ...
    Matrix.Log('BLACKMARKET', '[SATIS] %s -> silah %s (seri:%s) $%.0f', citizenid, entry.label, weaponSerial, entry.price)
end)

-- =====================================================================
-- SATIN ALMA: YEDEK NAMLU
-- =====================================================================
...
-- =====================================================================
-- SATIN ALMA: AÇIK HAT (BURNER PHONE)
-- =====================================================================
