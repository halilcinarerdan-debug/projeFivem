-- =====================================================================
-- MATRIX MARKET / market.lua
-- Katman 5: QBOX Co-op Kartel Hiyerarşisi + Bölgesel Madde Piyasası +
-- Kılcal Damar Hardcore Mekanikler (Telsiz Sessizliği, Kirlenen Nakit
-- Sönümlenmesi, Undercover Ajan Yoğunlaşması).
--
-- Nesnesiz RAM Cache + dirty-set + async batch persistence mimarisi
-- Katman 1-4 ile AYNI disiplinde sürdürülür. RNG YOK: tüm eşikler/
-- formüller deterministiktir.
--
-- KAPSAM NOTLARI (bilinçli kararlar):
--   • "Ortak Risk Kontratı" (E_total / Katman 8 Hard-Wipe — çete-çapında
--     kümülatif kanıt eşiği aşılınca TÜM oyuncu verisinin aynı saniyede
--     DROP edilmesi) BU DOSYADA YOKTUR. İstek metninin kendisi bunu ayrı
--     bir "Katman 8" olarak işaretliyor; eşik/tetik/kapsam tanımsız.
--     Tanımsız bir toplu-silme mekanizmasını tahminle inşa etmek yerine
--     yalnızca rütbe/hiyerarşi TEMELİ (aşağıda) kuruldu.
--   • "Adli Panik" UI titremesi (client-side render efekti) bu kaynakta
--     HİÇ client_script YOKTUR (fxmanifest'te client_scripts bloğu yok) —
--     bu yüzden gerçek bir görsel titreme burada UYDURULMADI. Bunun yerine
--     PanicReplyDelayMs adlı SUNUCU-tarafı, ölçülebilir bir gecikme
--     sağlanır (cortisol_level'a bağlı); gerçek görsel efekt ayrı bir
--     client kaynağı geldiğinde bu gecikmenin üzerine inşa edilebilir.
--   • Toptancı↔Büro güven köprüsü (Matrix.Supplier.ApplyBureauIntelLeak)
--     bu dosyada DEĞİL, server/logistics.lua'da tamamlandı (trust makinesi
--     zaten orada yaşıyor; bkz. o dosyanın Katman 5 notu).
--   • /sevket, /filoata, /filobirak komutlarının rütbe guard'ı bu dosyada
--     DEĞİL server/logistics.lua'da eklendi (komutların kendisi orada
--     tanımlı); burada yalnızca Matrix.Hierarchy.HasCommandAuthority API'si
--     sağlanır, o dosya bunu KOŞULLU olarak çağırır.
-- =====================================================================
...
-- Ortak bot matrisine / filoya komuta yetkisi. Rank yoksa (hiyerarşiye hiç
-- girmemiş oyuncu) DAİMA false — varsayılan KISITLI, açık değil.
...
function Matrix.Hierarchy.SetRank(citizenid, rank, assignedBy)
    ...
    -- Rütbe ataması nadir bir yönetici eylemidir (dirty-set batching'e
    -- gerek yok) — CreateTrapHouse İLE AYNI şekilde doğrudan async upsert.
    MySQL.prepare([[
...
-- =====================================================================
local MarketZones      = {}   -- zoneId -> { zone_id, price_multiplier, rejected_streak }
    ...
    local callOk = pcall(function()
        MySQL.query('SELECT * FROM matrix_market_zones', {}, function(rows)
            pcall(function()
                ...
                -- Config'te tanimli ama DB'de satiri olmayan bolgeler icin varsayilan satir.
...
-- FORMÜL (Gurme Müşteri Reddi + Bölge Fiyat Çöküşü):
--   isGourmet = cognitive_shifter > GourmetCognitiveShifterThreshold
--   rejected  = isGourmet AND purity < GourmetMinPurity
--   (rejected iken) fiyat çarpanı Newton-soğuma tarzı üstel sönümlenmeyle
--   tabana doğru çöker — matrix_supplier_trust'ın PassiveTrustRecoveryPerRealDay
--   sürüklenmesiyle AYNI matematiksel aile:
--     decayRate = RejectionPriceDecayRate * DemandElasticity   (η<1 ise YAVAŞLAR)
--     multiplier' = Floor + (multiplier - Floor) * exp(-decayRate)
-- Yorum: Talep esnekliği (η) < 1 (inelastik) ise birkaç gurme müşterinin
-- reddi TÜM bölge fiyatını çökertmez — decayRate küçülür, çöküş yavaşlar.
-- RNG YOK; aynı (purity, cognitive_shifter) çifti HER ZAMAN aynı sonucu verir.
function Matrix.Market.EvaluateSale(zoneId, buyerCitizenid, buyerCognitiveShifter, purity, sellerBallisticId)
    ...
    -- ★ Undercover ajan tetiği (bkz. bölüm 5). sellerBallisticId verilmişse
    -- ve alıcı gizli ajansa, ilgili balistik kaydı otomatik %100 mühürlenir.
...
-- Dış kaynaklar (bir satış/teslimat sistemi) bu event üzerinden EvaluateSale'i
-- tetikleyebilir. botId üzerinden en yakın piyasa bölgesi çözülür; buyer kimliği
-- bu event'i TETİKLEYEN src'nin citizenid'idir (gerçek bir satış noktası kaynağı
-- daha kesin bir alıcı kimliği ile doğrudan export'u çağırmalıdır).
...
-- =====================================================================
local SilenceExpiry = {}   -- citizenid -> expiresAtEpoch
...
-- bureau.lua/main.lua gibi yalnızca `src` (number) elinde olan çağıranlar
-- için kolaylık sarmalayıcı.
...
-- =====================================================================
-- 4) KİRLENEN NAKİT SÖNÜMLENMESİ (Cash Decay)
-- =====================================================================
local CashByTrapHouse = {}   -- trapHouseId -> { dirty_amount, deposited_at(epoch) }
local dirtyCash        = {}
                ...
                if type(rows) == 'table' then
                    for _, row in ipairs(rows) do
                        if row and row.trap_house_id then
                            -- logistics.lua'nin SupplierTrustCache.last_touched'i İLE AYNI
                            -- disiplin: restart anindaki yas sifirlanir (yalnizca dirty_amount
                            -- kalicidir) — SQL DATETIME'i epoch'a geri cozen kirilgan bir
                            -- parser eklemek yerine bilincli bir basitlestirme.
...
-- Aklama: dirty_amount'tan dogrudan duser; TAMAMEN sifirlanirsa iz yasi da
-- sifirlanir (yeni bir suc dongusu basliyor demektir). Kismi aklamada yas
-- KORUNUR — kismi odeme kismi temizlik demektir, ani "reset" yoktur.
...
-- FORMÜL (iz seviyesi — τ=TraceHalfLifeRealDays GERÇEK yarı-ömür tanımıyla):
--   ageDays     = (now - deposited_at) / 86400
--   trace_level = 1 - 0.5^(ageDays / TraceHalfLifeRealDays)    ∈ [0,1)
--   raid_gain   = Bureau.PatternAnalysisGain * trace_level * (RaidRiskMultiplierAtMaxTrace - 1.0)
-- Yorum: trace_level tam τ günde 0.5'e ulaşır (adının gereği); asla 1'e
-- eşitlenmez, sonsuza yaklaşır. raid_gain, Büro'nun VAR OLAN pasif desifre
-- formülüne (PatternAnalysisGain) ÜSTÜNE binen bir bonustur — Matrix.Bureau.
-- AdvanceDecryption üzerinden uygulanır; Büro'nun kendi formülü HİÇ değişmedi.
...
-- =====================================================================
local UndercoverFlags = {}   -- citizenid -> true

function Matrix.Undercover.IsUndercoverAgent(citizenid)
    return type(citizenid) == 'string' and UndercoverFlags[citizenid] == true
end

-- FORMÜL (şüphe endeksi, [0,1]'e clamp'li, RNG YOK):
--   suspicion = times_reported*SuspicionReportWeight + completed_deals*SuspicionDealWeight
-- SuspicionDealWeight NEGATİFTİR: çok sayıda başarılı iş bir ajanı GERÇEK
-- müşteriden ayırt eder (gerçek bir ajan operasyonu tehlikeye atacak kadar
-- çok "iş" biriktirmez) — şüpheyi AZALTIR. Tarama SADECE propaganda
-- momentumu InfiltrationMomentumThreshold'u aştığında çalışır (bütçesiz/
-- organik sosyal medya baskısı olmadan sızma yoktur).
