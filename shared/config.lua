...
-- KILCAL DAMAR HARDCORE MEKANİKLER  (bkz. server/market.lua)
--
-- NOT: "Ortak Risk Kontratı" (E_total / Katman 8 Hard-Wipe) bu revizyonda
-- KASITLI OLARAK YOK — istek metninin kendisi bunu "Katman 8" olarak
-- işaretliyor ve eşik/tetik/kapsam tanımlı değil. Tanımsız bir toplu-silme
-- mekanizmasını tahminle inşa etmek yerine sadece rütbe/hiyerarşi temeli
-- (bu bölüm) kuruldu; Hard-Wipe kendi katmanı netleştiğinde eklenmelidir.
-- =====================================================================
Config.Hierarchy = {
    Ranks = {
    ...
    -- Ortak bot matrisine/filoya komuta (bkz. /sevket) için gereken minimum
    -- rütbe seviyesi. Varsayılan 2: Leader + Logistics_Officer (rütbe adı
    -- zaten "lojistik" diyor) sevk emri verebilir; Chemist mutfağı yönetir,
    -- sevk yetkisi YOKTUR. Tek bir sayı değiştirilerek tasarımcı bunu
    -- gevşetebilir/sıkılaştırabilir.
...
Config.Market = {
    ...
    -- Gurme müşteri: cognitive_shifter bu eşiğin ÜSTÜNDEYSE ve teslim edilen
    -- saflık GourmetMinPurity'nin ALTINDAYSA parti reddedilir.
    GourmetCognitiveShifterThreshold = 0.7,
    GourmetMinPurity                 = 0.30,

    -- Reddedilen parti biriktikçe bölge fiyat çarpanı Newton-soğuma tarzı
    -- üstel sönümlenmeyle tabana doğru çöker (bkz. matrix_supplier_trust'ın
    -- PassiveTrustRecoveryPerRealDay'i İLE AYNI matematiksel aile):
    --   multiplier' = Floor + (multiplier - Floor) * exp(-DecayRate * Elasticity)
    -- Talep esnekliği (η) < 1 ise (inelastik) çarpanı YAVAŞLATIR — birkaç
    -- gurme müşterinin reddi tüm bölge fiyatını çökertmez.
...
-- ★ Evrilen Balistik (Gauss Çekirdeği Mutasyonu) — bkz. forensics.lua.
-- CasingWearWeight/CasingCortisolWeight ZATEN Config.Forensics'te (0.3/0.2)
-- tanımlıydı; istenen formülün ağırlıkları bunlarla BİREBİR aynı olduğundan
-- yeni bir sabit eklenmedi, sadece durability çarpanı için yenileri eklendi.
...
Config.CashDecay = {
    -- τ = 90 gün: dirty_amount ne kadar sürede sıfırlanmazsa iz seviyesi
    -- (trace_level) asimptotik olarak 1.0'a yaklaşır (bkz. market.lua
    -- ApplyOrganicSkillGrowth İLE AYNI matematiksel aile — 1-exp(-t/τ)).
    TraceHalfLifeRealDays         = 90.0,
    -- İz seviyesi bu değerle Büro'nun trap house deşifre kazancını çarpar
    -- (trace_level=1.0 iken TAM olarak bu katsayı uygulanır).
    RaidRiskMultiplierAtMaxTrace  = 2.0,
    -- Paravan şirkete enjekte edilen (aklanan) her miktar dirty_amount'tan
    -- doğrudan düşülür; iz seviyesi YENİDEN başlamaz (kısmi ödeme kısmi
    -- temizlik demektir — ani "reset" yok).
    LaunderReducesAmount          = true,
    TickIntervalMs                = 60000
}

Config.Undercover = {
    -- Bütçesiz (organik) propaganda momentumu bu eşiği geçtiğinde müşteri
    -- havuzu gizli ajan sızması için taranır.
    InfiltrationMomentumThreshold = 3.0,
    -- Şüphe formülü: suspicion = times_reported*ReportWeight + completed_deals*DealWeight
    -- (DealWeight NEGATİF: çok sayıda başarılı iş bir ajanı gerçek müşteriden
    -- ayırt eder, şüpheyi AZALTIR). [0,1]'e clamp'lenir.
