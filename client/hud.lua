-- =====================================================================
-- MATRIX HUD / client/hud.lua  (KATMAN 5 ULTIMATE — ATMOSFERİK EVRİM)
...
--   [U1] F10 "/sevket" butonu TAMAMEN KALDIRILDI (yalnızca bu dosyadan —
--        sunucudaki /sevket komutu geriye dönük uyum için dokunulmadan
--        kalıyor). "Rota Çiz" tek fiziksel sevk arayüzü olarak kalıyor;
--        varışta otomatik stash teslimatı server/main.lua'da yapılır.
--   [U2] Canlı Kadro raporu artık TIKLANABİLİR: her bot satırı "Operatif
--        Tasfiye Et" ve (uygunsa) "Bölge Denetleyicisi Olarak Ata" alt
--        menüsünü açar (bkz. OpenBotActionsMenu). Callback artık yapılı
--        `{kind,id,role,mole_flagged,text}` dizisi döner (server/main.lua).
--   [U3] Mekanik Tutukluk: mermi sayısı azalma tespiti (IsPedShooting
--        yerine ammo-delta polling — tam otomatik seride daha güvenilir)
--        her atışta sunucuya bildirilir. Sunucu tutukluk bildirirse ateş
--        etme yetisi (DisablePlayerFiring) anında bloke edilir ve kırmızı
--        [MEKANİK: SİLAH TUTUKLUK YAPTI] uyarısı basılır; 'X' tuşu veya
--        F10'dan 6 saniyelik lib.progressCircle tahliyesi başlatılabilir.
--   [U4] Karaborsa Ticaret Ağı + Bölgesel Mali Rapor F10 alt menüleri.
...
CreateThread(function()
    while true do
        ...
        if weaponHash and weaponHash ~= 0 and weaponHash ~= `WEAPON_UNARMED` then
...
--- ★ [U5] Herhangi bir telefon kaynağının çağrı başlangıcı/bitişinde
...
local function OpenRotaCizDialog()
    local input = lib.inputDialog('/rotaciz - Multi-Waypoint Taktik Rota (Otomatik Intikal)', {
        ...
        },
        {
            ...
            description = '"x,y,z" veya "x y z" VEYA Trap House ID — varista kargo otomatik depoya aktarilir',
...
local function OpenBlackMarketVehicles()
    ...
    for _, v in ipairs(Config.BlackMarket and Config.BlackMarket.Vehicles or {}) do
        options[#options + 1] = {
            ...
            icon        = 'car-side',
            onSelect    = function() TriggerServerEvent('matrix:server:blackmarket:buyVehicle', v.id) end
        }
    end
...
local function OpenBlackMarketWeapons()
    ...
    for _, w in ipairs(Config.BlackMarket and Config.BlackMarket.Weapons or {}) do
        options[#options + 1] = {
            title       = w.label,
            description = ('Fiyat: $%d | Baslangic Asinma: %.0f%%'):format(math.floor(w.price), w.durability),
            icon        = 'gun',
            onSelect    = function() TriggerServerEvent('matrix:server:blackmarket:buyWeapon', w.id) end
        }
    end
...
local function OpenBlackMarketBarrelAndPhones()
    local options = {
        {
            ...
            onSelect    = function() TriggerServerEvent('matrix:server:blackmarket:buySpareBarrel') end
        }
    }
    for _, p in ipairs(Config.BlackMarket and Config.BlackMarket.BurnerPhones or {}) do
        options[#options + 1] = {
            ...
            icon        = 'mobile-screen',
            onSelect    = function() TriggerServerEvent('matrix:server:blackmarket:buyBurnerPhone', p.id) end
        }
    end
    ...
    lib.registerContext({
        ...
        options = {
            ...
            },
            {
                ...
                description = 'Seri no silinmis, asinmis silahlar',
                icon        = 'gun',
...
RegisterNetEvent('matrix:client:blackmarket:purchaseResult', function(ok, label, ref)
    if lib and lib.notify then
        lib.notify({
            title       = '[KARABORSA]',
            description = ok and ('%s teslim alindi.'):format(tostring(label)) or ('%s satin alinamadi.'):format(tostring(label)),
...
OpenTacticalMenu = function()
    lib.registerContext({
        ...
        options = {
            ...
            {
                title       = 'Karaborsa Ticaret Agi',
                description = 'Illegal arac filosu, silah kacakciligi, yedek namlu, acik hat',
